                    ...
                    <table class="table table-nowrap mb-2">                      
                      <thead>
                        ...
                        <tr>
                          <th>COURSE</th>
                          <th width="150"></th>
                          <th width="150" class="text-center">Price (Rs.)</th>
                      ...
                      <tbody>';
                        ...
                        if($WISHLIST){
                          foreach ($WISHLIST as $row) {
                            ...
                            // ADMOFF_AMOUNT
                            $condition = array (
                                                   'select'       =>	'admoff_amount' 
                                                  ,'where' 		    =>	array( 
                                                                             'admoff_status'  => 1
                                                                            ,'admoff_type'    => cleanvars($type)
                                                                            ,'admoff_degree'  => cleanvars($idWish)
                                                                          ) 
                                                  ,'return_type'	=>	'single'
                                                ); 
                            $ADMISSION_OFFERING = $dblms->getRows(ADMISSION_OFFERING,$condition, $sql);
                            $total += $ADMISSION_OFFERING['admoff_amount'];
                            echo '
                            <tr>
                              <td>
                                ...
                                <input type="hidden" class="removable" name="amount['.$i.']" value="'.$ADMISSION_OFFERING['admoff_amount'].'">
                              </td>
                              <td class="text-center">'.get_enroll_type($type).'</td>
                              <td class="text-center">'.$ADMISSION_OFFERING['admoff_amount'].'</td>
                        ...
                        if(!$found){
                          ...
                          if($_POST['type'] == 1){
                            $condition['select'] = 'ao.admoff_degree, ap.program, p.prg_photo, ao.admoff_amount';
                            $condition['join'] = 'LEFT JOIN '.ADMISSION_PROGRAMS.' ap ON ap.id = ao.admoff_degree
                                                  LEFT JOIN '.PROGRAMS.' p ON p.prg_id = ap.id_prg';
                          }elseif($_POST['type'] == 2){
                            $condition['select'] = 'ao.admoff_degree, m.mas_name, m.mas_photo, ao.admoff_amount';
                            $condition['join'] = 'LEFT JOIN '.MASTER_TRACK.' m ON m.mas_id = ao.admoff_degree';
                          }elseif($_POST['type'] == 3){
                            $condition['select'] = 'ao.admoff_degree, c.curs_name, c.curs_photo, ao.admoff_amount';
                        ...
                        <tr>
                          <th>
                            <div class="form-switch text-primary">
                              ...
                              <label class="form-check-label mb-0">Have Coupon Code?</label>
                            </div>
                          </th>
                          <th class="text-end">
                            Grand Total:
