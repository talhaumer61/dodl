...
$con    = array ( 
                    ...
                    ,'join'     =>	'LEFT JOIN '.LECTURE_TRACKING.' AS lt ON ( lt.id_curs = '.$COURSES['curs_id'].' AND lt.id_lecture = cl.lesson_id AND lt.id_std = '.cleanvars($_SESSION['userlogininfo']['STDID']).' AND lt.id_mas = '.$COURSES['id_mas'].' AND lt.id_ad_prg = '.$COURSES['id_ad_prg'].') AND lt.is_deleted = 0' 
                    ,'where' 	=>	array( 
                                                'cl.lesson_status' => 1
                                            ,'cl.is_deleted'    => 0
                    ...
                    ,'order_by'     =>	'cl.id_week, cl.id_lecture, cl.lesson_ordering ASC LIMIT 1'
                    ,'return_type'  =>	'single'
                ); 
...
$con    = array ( 
                    ...
                    ,'join'     =>	'LEFT JOIN '.LECTURE_TRACKING.' AS lt ON (lt.id_curs = '.cleanvars($COURSES['curs_id']).' AND lt.id_assignment = ca.id AND lt.id_std = '.cleanvars($_SESSION['userlogininfo']['STDID']).' AND lt.id_mas = '.$COURSES['id_mas'].' AND lt.id_ad_prg = '.$COURSES['id_ad_prg'].') AND lt.is_deleted = 0' 
                    ,'where' 	=>	array( 
                                                'ca.status'        => 1
                                            ,'ca.is_deleted'    => 0
...
$con    = array ( 
                    ...
                    ,'join'     =>	'LEFT JOIN '.LECTURE_TRACKING.' lt ON (lt.id_quiz = q.quiz_id AND lt.id_curs = '.$COURSES['curs_id'].' AND lt.id_std = '.$_SESSION['userlogininfo']['STDID'].' AND lt.id_mas = '.$COURSES['id_mas'].' AND lt.id_ad_prg = '.$COURSES['id_ad_prg'].') AND lt.is_deleted = 0'
                    ,'where' 	=>	array( 
                                                'q.quiz_status'    => 1
                                            ,'q.is_deleted'     => 0
                                            ,'q.is_publish'     => 1
...
echo'
<div '.($quizLocked ? 'hidden' : 'class="sidebar"').'>
            ...
            <div uk-accordion>';                        
                ...
                foreach(get_LessonWeeks() as $Weekkey => $WeekVal):
                    ...
                    $con = array(
                                    ...
                                    ,'join'         =>	'LEFT JOIN '.LECTURE_TRACKING.' AS lt ON (lt.id_curs = '.$COURSES['curs_id'].' 
                                                            AND lt.id_lecture = l.lesson_id AND lt.id_std = '.$_SESSION['userlogininfo']['STDID'].' 
                                                            AND lt.id_mas = '.$COURSES['id_mas'].' AND lt.id_ad_prg = '.$COURSES['id_ad_prg'].') AND lt.is_deleted = 0'
                                    ,'where'        =>  array(
                                    ...
                                    ,'order_by'     =>  'l.id_week ASC, l.id_lecture ASC, l.lesson_ordering ASC'
                                    ,'return_type'  =>  'all'
                                );
                    ...
                    $con = array(
                                    ...
                                    ,'join'         => 'LEFT JOIN '.LECTURE_TRACKING.' AS lt ON (lt.id_curs = '.$COURSES['curs_id'].' AND lt.id_assignment = a.id AND lt.id_std = '.$_SESSION['userlogininfo']['STDID'].' AND lt.id_mas = '.$COURSES['id_mas'].' AND lt.id_ad_prg = '.$COURSES['id_ad_prg'].') AND lt.is_deleted = 0'
                                    ,'where'        => array(
                    ...
                    $con    = array(
                                    ...
                                    ,'join'         => 'LEFT JOIN '.LECTURE_TRACKING.' AS lt ON (lt.id_curs = '.$COURSES['curs_id'].' AND lt.id_quiz = q.quiz_id AND lt.id_std = '.$_SESSION['userlogininfo']['STDID'].' AND lt.id_mas = '.$COURSES['id_mas'].' AND lt.id_ad_prg = '.$COURSES['id_ad_prg'].') AND lt.is_deleted = 0'
                                    ,'where'        => array(
                                                                    'q.quiz_status'  => 1
                                                                ,'q.is_publish'   => 1
                                                                ,'q.is_deleted'   => 0
                    ...
                    if ($COURSES_LESSONS || $COURSES_ASSIGNMENTS || $QUIZ) {                                             
                        ...
                        <div class="'.(($tab_open_flag)?'uk-open':'').' pt-2 pb-2 border-bottom">
                            <a class="uk-accordion-title text-md mx-2 font-semibold" href="#">
                                <div class="mb-1 text-sm font-medium"></div> 
                                '.moduleName(get_CourseWise($COURSES['curs_wise'])).' '.$COURSES_LESSONS[0]['id_week'].'
                                        ...
                                        <div class="card">
                                            ...
                                            <div class="card-body">';
                                                foreach ($COURSES_LESSONS AS $Lskey => $LsVal) {
                                                    ...
                                                    $is_completed   = ((!empty($LsVal['is_completed']) && $LsVal['is_completed'] == 2) ? 'far fa-check-circle text-success' : '');
                                                    ...
                                                    $cursor        .= ($LsVal['is_completed'] == 2 ? ' background: #15ae1524;' : '');
                                                    $cursor        .= ($next_id == $LsVal['lesson_id'] ? ' background: #CFE2FF;' : '');
                                                    echo'
                                                    <li class="'.(($LsVal['lesson_id'] == $slug) ? 'uk-active' : '').'">
                                                        <a '.$href.' style="'.$cursor.'">
                                                            ...
                                                            <span>
                                                                <i style="font-size:20px;" class="'.$is_completed.'"></i>
                                                            </span>
                                                        </a>
                                                    </li>';
                                                }
                                        ...
                                        <div class="card">
                                            ...
                                            <div class="card-body">';
                                                foreach ($COURSES_ASSIGNMENTS AS $Lskey => $LsVal) {
                                                    ...
                                                    $cursor        .= ($LsVal['is_completed'] == 2 ? ' background: #15ae1524;' : '');
                                                    $cursor        .= ($next_id == $LsVal['id'] ? ' background: #CFE2FF;' : '');
                                                    echo'
                                                    <li class="'.(($LsVal['id'] === $slug) ? 'uk-active' : '').'">
                                                        <a '.$href.' style="'.$cursor.'">
                                                            <i style="font-size:20px;" class="fas fa-clipboard"></i>
                                                            <h6 class="mx-1 mb-0">'.($Lskey+1).'-'.html_entity_decode($LsVal['caption']).'</h6> 
                                                            <span> 
                                                                <i style="font-size:20px;" class="'.$is_completed.'"></i>
                                                            </span>
                                                        </a>
                                        ...
                                        <div class="card">
                                            ...
                                            <div class="card-body">';
                                                foreach ($QUIZ AS $Lskey => $LsVal) {
                                                    ...
                                                    $cursor        .= ($LsVal['is_completed'] == 2 ? ' background: #15ae1524;' : '');
                                                    $cursor        .= ($next_id == $LsVal['quiz_id'] ? ' background: #CFE2FF;' : '');
                                                    echo'
                                                    <li class="'.(($LsVal['quiz_id'] === $slug) ? 'uk-active' : '').'">
                                                        <a '.$href.' style="'.$cursor.'">
                                                            <i style="font-size:20px;" class="fas fa-paperclip"></i>
                                                            <h6 class="mx-1 mb-0">'.($Lskey+1).'-'.html_entity_decode($LsVal['quiz_title']).'</h6> 
                                                            <span> 
                                                                <i style="font-size:20px;" class="'.$is_completed.'"></i>
                                                            </span>
                                                        </a>
