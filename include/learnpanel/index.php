...
if(!empty($zone)) {
    ...
    if($slug == '0'){
        $con    = array ( 
                            ...
                            ,'order_by'     =>	'cl.id_week, cl.id_lecture, cl.lesson_id ASC'
                            ,'return_type'  =>	'single'
                        ); 
    ...
    $con = array(
                    ...
                    ,'join'         =>  'INNER JOIN '.COURSES.' AS c ON FIND_IN_SET(c.curs_id,ec.id_curs) AND c.curs_href = "'.cleanvars($zone).'" AND c.curs_status = 1 AND c.is_deleted = 0
                                        ...
                                        LEFT JOIN '.LECTURE_TRACKING.' AS lt ON lt.id_curs = c.curs_id AND lt.id_std = '.cleanvars($_SESSION['userlogininfo']['STDID']).' AND lt.is_completed = 2 AND lt.is_deleted = 0 AND lt.id_mas = ec.id_mas AND lt.id_ad_prg = ec.id_ad_prg AND lt.is_deleted = 0'
                    ,'where'        =>  array(
                                                 'ec.secs_status'   => 1
                                                ,'ec.is_deleted'    => 0    
    ...
    if ($COURSES) {
        ...
        $quizLocked = isset($_SESSION['QUIZ_ACTIVE']);
