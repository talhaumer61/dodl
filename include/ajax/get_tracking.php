...
if ($_POST['track_mood'] === 'playing') {
    ...
    if ($chk) {
        if ($chk['is_completed'] == 1) {
            ...
            $response = 'OK:TRACK_EXISTS';
        } else {
            $response = 'OK:TRACK_SKIP';
        }
    } else {
        $values = array(
                         'track_status'     =>	1
        ...
        if ($sqllms) {
            ...
            $response = 'OK:TRACK_ADDED';
        } else {
            $response = 'ERR:DB_INSERT_FAILED';
        }
    }
    echo $response;
    exit;
...
if ($_POST['track_mood'] === 'completed') {
    ...
    if ($sqllms) {
        ...
        $con    = array ( 
                            ...
                            ,'join'     =>	'LEFT JOIN '.LECTURE_TRACKING.' AS lt ON ( lt.id_curs = '.$_POST['id_curs'].' AND lt.id_lecture = cl.lesson_id AND lt.id_std = '.cleanvars($_SESSION['userlogininfo']['STDID']).' AND lt.id_mas = '.$_POST['id_mas'].' AND lt.id_ad_prg = '.$_POST['id_ad_prg'].') AND lt.is_deleted = 0' 
                            ,'where' 	=>	array( 
                                                     'cl.lesson_status' => 1
                                                    ,'cl.is_deleted'    => 0
        ...
        $con    = array ( 
                            ...
                            ,'join'     =>	'LEFT JOIN '.LECTURE_TRACKING.' AS lt ON (lt.id_curs = '.cleanvars($_POST['id_curs']).' AND lt.id_assignment = ca.id AND lt.id_std = '.cleanvars($_SESSION['userlogininfo']['STDID']).' AND lt.id_mas = '.$_POST['id_mas'].' AND lt.id_ad_prg = '.$_POST['id_ad_prg'].') AND lt.is_deleted = 0' 
                            ,'where' 	=>	array( 
                                                     'ca.status'        => 1
                                                    ,'ca.is_deleted'    => 0
        ...
        $con    = array ( 
                            ...
                            ,'join'     =>	'LEFT JOIN '.LECTURE_TRACKING.' lt ON (lt.id_quiz = q.quiz_id AND lt.id_curs = '.$_POST['id_curs'].' AND lt.id_std = '.$_SESSION['userlogininfo']['STDID'].' AND lt.id_mas = '.$_POST['id_mas'].' AND lt.id_ad_prg = '.$_POST['id_ad_prg'].') AND lt.is_deleted = 0'
                            ,'where' 	=>	array( 
                                                     'q.quiz_status'    => 1
                                                    ,'q.is_deleted'     => 0
                                                    ,'q.is_publish'     => 1
        ...
        echo '<a href="'.SITE_URL.'learn/'.$_POST['curs_href'].'/'.$redirection.'" class="btn btn-sm btn-success px-4 py-1 rounded">NEXT '.moduleName($type).': '.html_entity_decode($name).'</a>';
    }
}
if ($_POST['track_mood'] === 'reading_metrail') {
    ...
    if (!$chk) {
        ...
        if ($sqllms) {
            ...
            echo '<a href="'.SITE_URL.'learn/'.$_POST['curs_href'].'/'.$redirection.'" class="btn btn-sm btn-success px-4 py-1 rounded">Next '.moduleName($type).': '.$name.'</a>';
        }   
    } else {
...
if (isset($_POST['_method']) && $_POST['_method'] === 'student_teacher_qna') {
