...
class dblms {
	...
	public function open_connectionlms() {
		try	{
            ...
            // mysqli_set_charset($this->connectlms, "utf8mb4");
	...
	*/
	public function Insert($table, $data) {
		...
		// Add default values if columns exist
		if (!array_key_exists($date_added, $data) && !array_key_exists($id_added, $data)) {
			$colName = $this->querylms("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '" . $table . "'");
			if (mysqli_num_rows($colName) > 0) {
				while ($row = mysqli_fetch_array($colName)) {
		...
		// Clean and check for SQL injection
		foreach ($data as $key => $val) {
			$cleaned = cleanvars($val);  // Check each value for suspicious input
			if ($cleaned === false) {  // If any value is suspicious, stop the insert
				$_SESSION['msg']['title'] = 'Error';
				$_SESSION['msg']['text'] = '❌ Blocked suspicious input.';
				$_SESSION['msg']['type'] = 'danger';
				
				// Reload the page and exit the function
		...
		// Prepare Insert using prepared statements
		$this->open_connectionlms();
		$fields = array_keys($data);
		$placeholders = implode(',', array_fill(0, count($fields), '?'));
		$types = str_repeat('s', count($fields)); // assume all are strings for simplicity
	
		$stmt = $this->connectlms->prepare("INSERT INTO $table (" . implode(',', $fields) . ") VALUES ($placeholders)");
	
		if ($stmt === false) {
			die("❌ MySQL Prepare failed: " . $this->connectlms->error);
		}
	...
	public function Update($table_name, $form_data, $where_clause='') {   
		...
		// Check each value for SQL injection and clean it
		foreach ($form_data as $key => $value) {
			$cleaned = cleanvars($value);  // Check each value for suspicious input
			if ($cleaned === false) {  // If any value is suspicious, stop the update
				$_SESSION['msg']['title'] = 'Error';
				$_SESSION['msg']['text'] = 'Blocked suspicious input.';
				$_SESSION['msg']['type'] = 'danger';
			...
			$form_data[$key] = $cleaned;  // Update the value with the cleaned one
		}
