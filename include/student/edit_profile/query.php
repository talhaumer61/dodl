...
if (isset($_POST['update_profile_pic'])) {
    ...
    if (!empty($_FILES['profile_pic']['name'])) {
        ...
        if ($fileSize > (300 * 1024)) {
            sessionMsg("Error!", "Image must be under 300KB", "danger");
            header("location: ".SITE_URL.CONTROLER);
            exit;
        }

        // 2. MIME check (real type)
        if (!in_array($fileType, ['image/jpeg', 'image/png'])) {
            sessionMsg("Error!", "Invalid image type", "danger");
            header("location: ".SITE_URL.CONTROLER);
            exit;
        }
        ...
        if ($imgInfo[0] !== $imgInfo[1]) {
            sessionMsg("Error!", "Image must be square", "danger");
            header("location: ".SITE_URL.CONTROLER);
            exit;
        }
...
if (isset($_POST['social_profile'])) {
    $values = array(
                         'std_sociallinks'      => json_encode($_POST['social'], JSON_UNESCAPED_UNICODE)
                        ,'id_modify'            => cleanvars($_SESSION['userlogininfo']['LOGINIDA'])            
                        ,'date_modify'          => date('Y-m-d G:i:s')
                    );
