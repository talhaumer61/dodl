                        ...
                        if($row){
                            ...
                            foreach ($row as $value) {
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">'.($value['status'] != 1 ? '' : date('d M, Y',strtotime($value['paid_date']))).'</td>
                                    ...
                                    <td class="text-center">';
                                        ...
                                        // }
                                        if($value['status'] == '2'){
                                            if($value['currency_code'] == 'PKR' && $value['total_amount']>0){
                                                echo '<a class="btn btn-info btn-sm text-white" href="'.SITE_URL.'raastpay/'.get_dataHashingOnlyExp($value['challan_no'], true).'" target="_blank"><i class="fa fa-money"></i> QR / Raast Payment</a>';
                                            }
                                            if( $value['total_amount']>0) {
                                                echo $payNow = '
                                            
                                            <form class="mt-1" action="' . SITE_URL . 'payfast" method="POST">
