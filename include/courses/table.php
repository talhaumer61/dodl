...
$condition = array ( 
                     'select' 		  =>	'c.curs_id, c.curs_type_status, c.curs_name, c.curs_wise, c.duration, c.curs_photo, c.curs_href, c.curs_hours, a.admoff_type, a.id_type, a.admoff_amount, a.admoff_amount_in_usd'
...
if($values){
  foreach ($values as $key => $course) {
    ...
    if (!empty($course['curs_photo'])) {
      $curs_photo = SITE_URL_PORTAL.'uploads/images/courses/'.$course['curs_photo'];
    }
