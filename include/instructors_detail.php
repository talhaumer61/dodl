...
if(!empty(ZONE)){
  // EMPLOYEE DETAIL
  $condition = array ( 
                          'select'       =>  'e.emply_gender, e.emply_id, e.emply_loginid, e.emply_photo, e.emply_introduction, e.emply_name, d.designation_name, e.emply_permanent_address, e.emply_phone, e.emply_email'
                          ,'join'         =>  'LEFT JOIN '.DESIGNATIONS.' d ON d.designation_id = e.id_designation'
                          ,'where' 	      =>  array ( 
                                                      'e.is_deleted'    => 0
                                                      ,'e.emply_status' => 1
                                                      ,'e.emply_href'   => cleanvars(ZONE)
                                              )
                          ,'return_type'  =>  'single'
                      ); 
  ...
  if($EMPLOYEES){
    ...
    $condition['join'] = 'INNER JOIN '.ALLOCATE_TEACHERS.' ct ON c.curs_id = ct.id_curs AND FIND_IN_SET('.$EMPLOYEES['emply_id'].', ct.id_teacher)
                          LEFT JOIN '.ENROLLED_COURSES.' ec ON FIND_IN_SET(c.curs_id, ec.id_curs)
                          INNER JOIN '.COURSES_CATEGORIES.' cc ON c.id_cat=cc.cat_id AND cc.id_type = 1';
    $countCOURSES = $dblms->getRows(COURSES.' c', $condition);
    ...
    $condition['join'] = 'INNER JOIN '.ALLOCATE_TEACHERS.' ct ON c.curs_id = ct.id_curs AND FIND_IN_SET('.$EMPLOYEES['emply_id'].', ct.id_teacher)
                          LEFT JOIN '.ENROLLED_COURSES.' ec ON FIND_IN_SET(c.curs_id, ec.id_curs)
                          INNER JOIN '.COURSES_CATEGORIES.' cc ON c.id_cat=cc.cat_id AND cc.id_type = 2';
    $countTRAININGS = $dblms->getRows(COURSES.' c', $condition);
    $condition['return_type'] = 'all';
    ...
    if(!empty($EMPLOYEES['emply_photo'])){
      $photo   = SITE_URL_PORTAL.'uploads/images/employees/'.$EMPLOYEES['emply_photo'];
    }
    echo'
          ...
          <div class="col-xl-12 col-md-12">
                    ...
                    <div class="card-body">
                      <div class="view-all-grp d-flex">
                        <div class="student-ticket-view">
                          <h3>'.(!empty($countCOURSES) ? $countCOURSES : 0).'</h3>
                          <p>Courses</p>
                        </div>
                ...
                <div class="col-lg-3 col-md-6">
                  <div class="card stat-info open-tickets">
                    <div class="card-body">
                      <div class="view-all-grp d-flex">
                        <div class="student-ticket-view">
                          <h3>'.(!empty($countTRAININGS) ? $countTRAININGS : 0).'</h3>
                          <p>Trainings</p>
                        </div>
                        <div class="img-deposit-ticket">
                    ...
                    if($COURSES){
                      foreach ($COURSES as $key => $value) {
                        ...
                        if(!empty($value['curs_photo'])){
                          $curs_photo   = SITE_URL_PORTAL.'uploads/images/courses/'.$value['curs_photo'];
                        }
                        echo'
            ...
            <div class="settings-widget">
              <div class="settings-inner-blk p-0">
                <div class="comman-space pb-0">
                  <div class="row">
                    <h5 class="subs-title text-left">Trainings</h5>';
                    if($TRAININGS){
                      foreach ($TRAININGS as $key => $value) {
                        $curs_rating = (($value['curs_rating'] == '0' || $value['curs_rating'] == '') ? '4' : $value['curs_rating']);
                      ...
                      echo'<h6 class="text-danger">No Trainings Found...!</h6>';
                    }
                    echo'
                  </div>
                </div>
    ...
    <div class="page-content course-sec p-0">
      <div class="container">
        <div class="row">
          <div class="col-lg-8">';
            if($EMPLOYEES['emply_introduction']){
              echo'
              <div class="card education-sec">
                <div class="card-body">
                  <h5 class="subs-title">Teacher Bio</h5>
                    <div class="edu-wrap mb-3">
                      <div class="edu-detail">
                        <div class="clamp-text" data-lines="10">
                          '.html_entity_decode(html_entity_decode($EMPLOYEES['emply_introduction'])).'
                        </div>
                      </div>
                    </div>
                </div>
