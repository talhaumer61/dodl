...
function checkCpanelLMSSTDLogin() {
	...
	// ✅ 2. Auto-login using Remember Me cookie (if session not already set)
	if (!isset($_SESSION['userlogininfo']) && isset($_COOKIE['remember_token'])) {
		$rememberToken = cleanvars($_COOKIE['remember_token']);
		...
		$remember_cond = array(
			...
			'where' 		=> array(
								 'a.adm_status' 		=> '1',
								 'a.is_deleted' 		=> '0',
								 'a.remember_token'		=> $rememberToken
							),
			'return_type' 	=> 'single'
		);

		$row = $dblms->getRows(ADMINS.' a', $remember_cond);
		if ($row) {
			...
			// ✅ Auto-login success: Rebuild session
			$_SESSION['userlogininfo'] = [
				'LOGINIDA'				=> $row['adm_id'],
				'LOGINTYPE'				=> $row['adm_type'],
				'LOGINAFOR'				=> $row['adm_logintype'],
				'LOGINUSER'				=> $row['adm_username'],
				'LOGINNAME'				=> $row['adm_fullname'],
				'LOGINEMAIL'			=> $row['adm_email'],
				'LOGINPHONE'			=> $row['adm_phone'],
				'LOGINPHOTO'			=> $adm_photo,
				'LOGINCAMPUS'			=> $row['id_campus'],
				'STDID'					=> $row['std_id'],
				'STDLEVEL'				=> $row['std_level'],
				'STDGENDER'				=> $row['std_gender'],
				'LOGINISTEACHER'		=> $row['is_teacher'],
				'EMPLYREQUEST'			=> $row['emply_request'],
				'LOGINORGANIZATIONID'	=> $row['id_org']
			];

			// Optionally: refresh remember_token validity
			$newToken = bin2hex(random_bytes(50));
			$dblms->Update(ADMINS, ['adm_remember_token' => $newToken], ['adm_id' => $row['adm_id']]);
			setcookie('remember_token', $newToken, time() + (60 * 60 * 24 * 365), "/", "", false, true);
...
function cpanelLMSSignInAsStudent() {
	...
	$login_remarks = 'WEB_LOGIN';
	$remember_me = isset($_POST['remember_me']); // ✅ check remember me
	...
	if (isset($_COOKIE['SWITCHTOSTUDENT']) && !empty($_COOKIE['SWITCHTOSTUDENT'])):
		...
		$login_remarks = 'MCDL_TO_DODL_SWITCH';
	endif;
	...
	if (!empty($adm_username) && !empty($adm_userpass)) {
		...
		if ($row) {
			...
			if($password == $row['adm_userpass']) {				
				// ✅ Remember Me Logic
				if ($remember_me) {
					$rememberToken = bin2hex(random_bytes(50));
					$updateData = array('remember_token' => $rememberToken);
					$dblms->Update(ADMINS, $updateData, "WHERE adm_id = '".$row['adm_id']."' ");

					// Set cookie for 1 year (HTTP-only)
					$cookie_expiry = time() + (60 * 60 * 24 * 365);
					setcookie('remember_token', $rememberToken, $cookie_expiry, "/", "", false, true);
				}
				...
				$dataLog = array(
									...
									,'remarks'			=> cleanvars($login_remarks)
...
function panelLMSSTDLogout() {
	...
	if (isset($_SESSION['userlogininfo']['LOGINIDA'])) {
		$userId = $_SESSION['userlogininfo']['LOGINIDA'];

		// ✅ 1. Clear remember_token from database
		$dblms->Update(ADMINS, ['remember_token' => NULL], " WHERE adm_id = '".$userId."' ");

		// ✅ 2. Destroy remember_token cookie
		setcookie('remember_token', '', time() - 3600, "/", "", false, true);
	}
	...
	// ✅ 4. Optional: restart session to show logout message
	session_start();
	sessionMsg("Logout", "You are logged out successfully.", "danger");

	// ✅ 5. Redirect to homepage or login page
	header("Location: " . SITE_URL);
	exit;
}