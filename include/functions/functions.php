...
function is_sql_injection_attempt($input) {
    $suspicious_patterns = [
        // SQL Injection
        '/\bUNION\b/i', '/\bSELECT\b/i', '/\bINSERT\b/i', '/\bUPDATE\b/i', '/\bDELETE\b/i',
        '/\bDROP\b/i', '/\bTABLE\b/i', '/\bWHERE\b/i', '/\bFROM\b/i', '/\bOR\b/i', '/\bAND\b/i',
        '/\bSLEEP\b/i', '/\bINFORMATION_SCHEMA\b/i',

        // SQL comment & control characters
        '/--/', '/#/', '/;/', '/\/\*/', '/\*\//', '/\bLIKE\b/i',
        ...
        '/\bcurl\b/i', '/\bwget\b/i', '/\bnslookup\b/i', '/\bping\b/i',
        '/\bexec\b/i', '/\bsystem\b/i', '/\beval\b/i', '/\bshell_exec\b/i',

        // Remote URL pattern
        '/\bhttp(s)?:\/\/[^ ]+/i', '/\.bxss\.me/i',

        // Encoded/obfuscated attacks
        '/%[0-9A-F]{2}/i', '/\\x[0-9A-Fa-f]{2}/',

        // XSS indicators
        '/<script\b/i', '/onerror\s*=|onload\s*=/i'
    ];
...
//    if (is_string($value) && is_sql_injection_attempt($value)) {
//        return false;
//    }
//
//    return $value;
...
function get_curs_status($id = '') {
	...
	if(!empty($id)){
		$curs_status = array (
										 '1'	=> '<span class="badge badge-danger">Exclusive</span>'
										...
										,'6'	=> '<span class="badge badge-warning">UK Pitman</span>'
									);
...
function get_LeanerType($id = '') {
	...
	if (!empty($id)) {
		$list	= [
					 '1' => '<span class="badge badge-orange">Paid</span>'
					,'2' => '<span class="badge badge-orange">Free</span>'
					,'3' => '<span class="badge badge-orange">Learn Free</span>'
		];
		return $list[$id];
	} else {
		return $list;
...
function getSocialMediaLinks() {
    return array(
        'facebook'  => array('name' => 'Facebook',  'icon' => 'fab fa-facebook-f',  'url' => 'https://www.facebook.com/muldodl/',  'color' => '#1877F2'),  // Facebook Blue
        'instagram' => array('name' => 'Instagram', 'icon' => 'fab fa-instagram',   'url' => 'https://www.instagram.com/dodl.mul/', 'color' => '#E1306C'),  // Instagram Pinkish
        'linkedin'  => array('name' => 'LinkedIn',  'icon' => 'fab fa-linkedin-in', 'url' => 'https://www.linkedin.com/',  'color' => '#0A66C2'),  // LinkedIn Blue
        'twitter'   => array('name' => 'Twitter',   'icon' => 'fab fa-twitter',     'url' => 'https://www.twitter.com/',   'color' => '#1DA1F2')   // Twitter Blue
    );
}
...
function validate_recaptcha()
{
    ...
    if (empty($_POST['g-recaptcha-response'])) {
        sessionMsg('CAPTCHA Required', '?? Please verify you are not a robot.', 'danger');
    ...
    if ($apiResponse === false) {
        sessionMsg('Connection Error', '?? Unable to validate reCAPTCHA.', 'danger');
        header("Location: " . $_SERVER['HTTP_REFERER']);
        exit;
    }
    ...
    // file_put_contents('captcha_debug.log', print_r($responseData, true), FILE_APPEND);
    ...
    if (empty($responseData['success']) || !$responseData['success']) {
        ...
        sessionMsg('CAPTCHA Failed', '? Verification failed: ' . $errors, 'danger');
        header("Location: " . $_SERVER['HTTP_REFERER']);
        exit;
    }
...
function get_learning_method($id = '') {
	$method_type = array (
						 '1' => 'Hybrid Learning - Online or classroom based'
						,'2' => 'Blended Learning - Online + live or in person support'
						,'3' => 'Self-Paced Learning - Learn anytime, anywhere at your own speed'
						,'4' => 'Instructor-Lead Training (ILT) - Live trainer sessions (online or in person)'
						,'5' => 'On-Demand Courses - Instant access, customizable for organizations/institutions'
						,'6' => 'Workshops/Bootcamps - Fast track and practical training'
						,'7' => 'Project-Based Learning - Online study with hands-on-practice and real projects'
					  );
	if($id != ''){
