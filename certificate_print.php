...
if(!empty(ZONE)){
    $conditions = array ( 
                         'select'       =>	'c.challan_id, c.status'
                        ,'join'         =>	'INNER JOIN '.CHALLANS.' c ON FIND_IN_SET(ec.secs_id, c.id_enroll)'
                        ,'where' 		=>	array( 
    ...
    $checkChallan = $dblms->getRows(ENROLLED_COURSES.' ec', $conditions); 
    if(isset($checkChallan) && $checkChallan['status'] == 1){
        $conditions = array ( 
        ...
        echo "else";
        header("Location: ".SITE_URL."learn-free-invoice/".get_dataHashingOnlyExp(ZONE,true)."");
    }
} else {
  $varx = 'home';
