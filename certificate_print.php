...
if(!empty(ZONE)){
    if($_SESSION['userlogininfo']['LOGINIDA']){
        $conditions = array ( 
                            'select'       =>	'c.challan_id, c.status, c.id_std'
                            ,'join'         =>	'INNER JOIN '.CHALLANS.' c ON FIND_IN_SET(ec.secs_id, c.id_enroll) AND c.is_deleted=0'
                            ,'where' 		=>	array( 
        ...
        $checkChallan = $dblms->getRows(ENROLLED_COURSES.' ec', $conditions, $sql);

        if($checkChallan && $checkChallan['id_std'] != $_SESSION['userlogininfo']['STDID']){
            $varx = 'home';
            sessionMsg('Error', 'You are trying to get other student\'s certificate', 'danger');
            echo'<script>window.location.href = "'.SITE_URL.'dashboard";</script>';
            exit();
        }
        
        if($checkChallan && $checkChallan['status'] == 1){
            $conditions = array ( 
        ...
        } elseif (isset($checkChallan) && $checkChallan['status'] == 2) {
            sessionMsg('Error', 'You have pending challan', 'danger');
            echo'<script>window.location.href = "'.SITE_URL.'student/challans";</script>';
            exit();
        } else {
            sessionMsg('Error', 'Certificate Fee is required', 'danger');
            echo'<script>window.location.href = "'.SITE_URL.'learn-free-invoice/'.get_dataHashingOnlyExp(ZONE,true).'";</script>';
            exit();
        }
    } else {
        $varx = 'home';
        sessionMsg('Error', 'You are not logged in', 'danger');
        echo'<script>window.location.href = "'.SITE_URL.'dashboard";</script>';
    }
} else {
  $varx = 'home';
  sessionMsg('Error', 'Enrollment Not Found', 'danger');
  echo'<script>window.location.href = "'.SITE_URL.'dashboard";</script>';
}
?>